...
export class Submodules {
  ...
  async getSubmodules(owner: string, repo: string, fromTag: string, toTag: string, paths: string[]): Promise<SubmoduleInfo[]> {
    ...
    for (const path of paths) {
      ...
      if (
        ...
        failOrError(`💥 Missing or couldn't resolve submodule path '${path}'.\n`, this.failOnError)
      }
    }
    core.endGroup()
    return modsInfo
